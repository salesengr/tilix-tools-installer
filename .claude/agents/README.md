...
A specialized set of 7 subagents tailored for bash-based security tool installation and maintenance.

## Current Branch Focus (recovery/tool-install-validation)

For this branch, prioritize **Intel Docker validation closure** before any broad refactoring:
- `scripts/docker_validate_tools.sh`
- `scripts/manifests/legacy_tools_d870fee.tsv`
- `docs/INTEL_VALIDATION_PLAN.md`

During this phase, prefer **small additive fixes** and targeted regressions over architecture changes.
...
### Workflow 1: Intel Validation Closure (Primary for this branch)

**Use case:** Remaining tools fail in Docker validation

```
Step 1: Planning
> Use planner to select next tool cohort and prerequisite strategy

Step 2: Snapshot prep
> Use bash-script-developer to create/update precheck snapshot run steps

Step 3: Targeted validation
> Use test-automation-engineer to run `scripts/docker_validate_tools.sh --tools ...` and capture logs

Step 4: Failure triage
> Use debugger to classify failure: installer logic, upstream package, timeout, or smoke-check mismatch

Step 5: Security + quality gates
> Use security-auditor and code-reviewer on any code changes

Step 6: Documentation
> Use documentation-engineer to update validation notes/results
```

### Workflow 2: Adding a New Security Tool (Post-validation phase)
...
### Workflow 3: Debugging Installation Failure
...
### Workflow 4: Security Audit Before Release
