...
### Workflow 0: Intel Validation Closure (Primary for recovery/tool-install-validation)

**Goal:** close remaining failures from `scripts/manifests/legacy_tools_d870fee.tsv` using Intel-native Docker and minimal-risk fixes.

```bash
# Step 1: Plan tool cohort + prerequisites
> Use planner to choose next cohort and prerequisite snapshot strategy

# Step 2: Snapshot prep
> Use bash-script-developer to prepare commands for precheck snapshots

# Step 3: Targeted validation
> Use test-automation-engineer to run:
> bash scripts/docker_validate_tools.sh --image <snapshot> --tools <csv> --log-dir <dir>

# Step 4: Triage failures
> Use debugger to classify: installer logic vs upstream issue vs timeout vs smoke-check mismatch

# Step 5: Patch minimally
> Apply smallest fix, rerun impacted tool + one same-ecosystem regression

# Step 6: Audit and document
> Use security-auditor + code-reviewer, then update docs/results
```

**Branch guardrails:**
- Prefer additive, minimal changes
- Do not broad-refactor during validation closure
- Keep `legacy_tools_d870fee.tsv` as source of truth

### Workflow 1: Adding a New Security Tool (Post-validation phase)
