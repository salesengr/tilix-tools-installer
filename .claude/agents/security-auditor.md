...
## Docker Validation Security Checks (Current Branch)

During Intel Docker validation, include:
- **Image provenance checks:** verify expected base image tag/digest before test runs
- **Log hygiene:** ensure `.tmp/docker-validation-*` logs do not contain credentials/tokens
- **Output discipline:** avoid writing secrets or host-sensitive data into shared artifacts
- **Change scope:** enforce minimal, auditable fixes during validation closure
