...
## Container Validation Mode (Current Branch Priority)

When on `recovery/tool-install-validation`, prefer manifest-driven Docker validation over ad-hoc local tests:

```bash
bash scripts/docker_validate_tools.sh \
  --image <snapshot-image> \
  --tools <tool1,tool2> \
  --log-dir .tmp/docker-validation-<timestamp>-<cohort>
```

Validation expectations:
- Ensure logs are created per tool in the selected log directory
- Record pass/fail by tool and include concise tails for failed logs
- Distinguish installer failure from smoke-check mismatch (install appears to run, command/path check fails)
- Keep reruns targeted to impacted tools + focused regression
