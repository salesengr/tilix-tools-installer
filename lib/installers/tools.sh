...
install_shodan() {
	...
	python_bin=$(_get_python_bin)
	local logfile
	logfile=$(create_tool_log "shodan")
	echo -e "${INFO}⚙ Installing shodan via pip --user...${NC}"
	{
		...
		if [ -f "$HOME/.local/bin/shodan" ]; then
			...
			if ! "$python_bin" -c "import pkg_resources" 2>/dev/null; then
				cat >"$HOME/.local/bin/shodan" <<WRAPPER_EOF || return 1
#!/usr/bin/env python3
import sys
import types
try:
...
WRAPPER_EOF
				chmod +x "$HOME/.local/bin/shodan" || return 1
			fi
		fi
		echo "Completed: $(date)"
	} >"$logfile" 2>&1
		...
		if [ ! -x "$HOME/.local/bin/yara" ]; then
			...
			if command -v autoreconf >/dev/null 2>&1; then
				...
				if download_file "$url" "$filename"; then
					...
					verify_sha256 "$filename" "${url}.sha256"
					_yara_rc=$?
...
install_rustscan() {
	...
	echo -e "${INFO}⬇ Installing rustscan (pre-built binary)...${NC}"
	{
		...
		asset_url=$(curl -fsSL "$api_url" 2>/dev/null |
			...
			head -1)
		if [[ -z "$asset_url" ]]; then
			echo "ERROR: asset not found (bee-san/RustScan latest)"
			return 1
		fi
		...
		local _rc
		verify_sha256 "rustscan.zip" "${asset_url}.sha256"
		_rc=$?
		[ "$_rc" -eq 1 ] && {
			rm -f rustscan.zip
			return 1
		}
		unzip -q rustscan.zip 2>/dev/null || {
			echo "ERROR: unzip failed for rustscan.zip"
			rm -f rustscan.zip
			return 1
		}
		local tgz
		...
		if [[ -n "$tgz" ]]; then
			tar -xzf "$tgz" 2>/dev/null || {
				echo "ERROR: tar extraction failed for $tgz"
		...
		if [[ -n "$bin" ]]; then
			...
			rm -f rustscan.zip "$tgz"
			echo "ERROR: rustscan binary not found"
			return 1
		fi
		rm -f rustscan.zip "$tgz"
		echo "Completed: $(date)"
...
install_sd() {
	...
	echo -e "${INFO}⬇ Installing sd (pre-built musl binary)...${NC}"
	{
		...
		local _rc
		verify_sha256 "sd-musl.tar.gz" "${asset_url}.sha256"
		_rc=$?
		[ "$_rc" -eq 1 ] && {
			rm -f sd-musl.tar.gz
			return 1
		...
		local found
		found=$(find . -name "sd" -type f ! -name "*.tar.gz" 2>/dev/null | head -1)
		if [[ -n "$found" ]]; then
			cp "$found" "$HOME/.local/bin/sd"
			chmod +x "$HOME/.local/bin/sd"
		else
			echo "ERROR: sd binary not found in musl archive"
			return 1
		fi
		rm -f sd-musl.tar.gz
		echo "Completed: $(date)"
	...
	if [ -x "$HOME/.local/bin/sd" ]; then
		...
		SUCCESSFUL_INSTALLS+=("sd")
		log_installation "sd" "success" "$logfile"
		cleanup_old_logs "sd"
		return 0
	fi
...
install_qtox() {
	...
	echo -e "${INFO}⬇ Installing qTox (AppImage extract mode)...${NC}"
	{
		...
		local _rc
		verify_sha256 "qtox.AppImage" "${asset_url}.sha256"
		_rc=$?
		[ "$_rc" -eq 1 ] && {
			rm -f qtox.AppImage
			return 1
