...
SERVEO_PORT=$(grep -oP '(?<=serveo\.net:)\d+' "${SERVEO_LOG}" 2>/dev/null || echo "")
SERVEO_URL=$(grep -oP 'tcp://\S+' "${SERVEO_LOG}" 2>/dev/null | head -1 || echo "see ${SERVEO_LOG}")
...
if [[ -n "${SERVEO_PORT}" ]]; then
    ...
    echo ""
    echo "  Test with:"
    echo "  curl -s -X POST http://serveo.net:${SERVEO_PORT} -d 'whoami'"
else
    echo "  Tunnel URL: ${SERVEO_URL}"
    echo "  Check log for port: cat ${SERVEO_LOG}"
fi
echo "=========================================="
